...
@parametrize(
    "value, confidence", [(-1, 0.5), (-0.5, 0.5), ("a", 0.5), (120.0, -1), (120.0, -0.5), (120.0, 2.0), (120.0, "a")]
)
...
def test_ns_note_hz_valid():
    ...
    confidences = np.linspace(0, 1.0, seq_len)
    confidences[seq_len // 2] = None  # throw in a None confidence value

    for t, d, v, c in zip(times, durations, values, confidences, strict=False):
...
def test_ns_pitch_hz_valid():
    ...
    times = np.arange(seq_len)
    durations = np.zeros(seq_len)
    values = np.linspace(-22050.0, 22050, seq_len)  # includes 0 (odd symmetric)
    confidences = np.linspace(0, 1.0, seq_len)
    confidences[seq_len // 2] = None  # throw in a None confidence value
...
def test_ns_note_midi_valid():
    ...
    times = np.arange(seq_len)
    durations = np.zeros(seq_len)
    values = np.linspace(-108.0, 108, seq_len)  # includes 0 (odd symmetric)
    confidences = np.linspace(0, 1.0, seq_len)
    confidences[seq_len // 2] = None  # throw in a None confidence value
...
def test_ns_contour_valid():
    ...
    for t, d, v, c, i, b in zip(times, durations, values, confidences, ids, voicing, strict=False):
...
@parametrize("value", ["asdf", "A&:phrygian", 11, "", ":dorian", None])
...
@parametrize("value", ["A:9", "Gb:sus2(1,3,5)", "X", "C:13(*9)/b7"])
...
@parametrize(
    "value",
    [f"{_}:maj" for _ in [42, "H", "a", "F1", True, None]]
    + [f"C:{_}" for _ in [64, "z", "mj", "Ab", "iiii", False, None]]
    + [f"C/{_}" for _ in ["A", 7.5, "8b"]]
    + [None],
)
...
@parametrize(
    "value",
    [f"{_}:maj" for _ in [42, "X", "a", "F1", True, None]]
    + [f"C:{_}" for _ in [64, "z", "mj", "Ab", "iiii", False, None]]
    + [f"C/{_}" for _ in ["A", 7.5, "8b"]]
    + [None],
)
...
@parametrize("event_duration", [0.5, 5, 1.0])
...
@parametrize("event_time", [0, 5, 1.0])
...
@parametrize("time_stretch", [0.5, 5, 1.0, None])
...
@parametrize("pitch_shift", [0.5, 5, 1.0, -1, -3.5, 0, None])
...
@parametrize("snr", [0.5, 5, 1.0, -1, -3.5, 0])
...
@parametrize("label", ["air_conditioner", "car_horn", six.u("street_music"), "any string"])
