...
def ann_contour():
    ...
    for t, v in zip(times, vibrato, strict=False):
        ann.append(time=t, duration=fs, value={"frequency": v, "index": 0, "voiced": (t < 3 or t > 4)})
...
@pytest.mark.parametrize("namespace, value", [("beat", 1), ("segment_open", "C"), ("onset", 1)])
@pytest.mark.parametrize("sr", [8000])
@pytest.mark.parametrize("duration", [2.0])
