...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("chord_harte", "1.0 2.0 a\n2.0 4.0 b", np.array([[1.0, 2.0], [2.0, 4.0]]), ["a", "b"], True),
        ("chord", "1.0 1.0 c\n2.0 2.0 d", np.array([[1.0, 2.0], [2.0, 4.0]]), ["c", "d"], False),
    ],
)
def test_import_lab(ns, lab, ints, y, infer_duration):
    ...
    for yi, ival, obs in zip(y, ints, ann, strict=False):
...
@pytest.mark.parametrize(
    "query, target", [("foo", "foo"), ("foo.txt", "foo"), ("/path/to/foo.txt", "foo"), ("/path/to/foo", "foo")]
)
...
def root_and_files():
    ...
    for fname, badfname in zip(files[::-1], badfiles[::-1], strict=False):
        os.remove(fname)
        os.remove(badfname)
...
def test_expand_filepaths():
    ...
    for search, result in zip(targets, paths, strict=False):
